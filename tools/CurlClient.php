...
class CurlClient implements ClientInterface
{
	 ...
	 * @inheritdoc
	 */
	public function getContentWithInfo($type = null,$ip = null, $port = null,$reffer)
	{
		$this->applySettings($type,$ip,$port,$reffer);
		$this->result = curl_exec($this->curl);
		$this->request_info = curl_getinfo($this->curl);
	...
	protected function applySettings($type = null,$ip = null, $port = null,$reffer = null)
	{
		if (empty($this->url)) {
	...
	public function parsePage($url,$type = null,$ip = null, $port = null,$reffer = null)
	{	
		$content = $this->setUrl($url)->getContentWithInfo($type,$ip,$port,$reffer);
